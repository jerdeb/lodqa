...
# Main
tree = etree.parse(open('lod-cloud_colored.svg','r'))
saveFile = 'lod-cloud-mod.svg'
...
for element in tree.iter():
    if element.tag.split("}")[1] == 'a':
        ...
        for child_elem in element.iter():
            if child_elem.tag.split('}')[1] == 'g':
                if (child_elem.get('id') == 'Oval' and child_elem.get('fill') is not None) :
                    observedID = URIRef(res + uri[26:])
                    
                    col = child_elem.get('fill');
                    
                    if (col == "#FEF1B6"):
                        category = Literal("linguistics", datatype=XSD.string)
                        graph_dump.add((observedID, obs.category, category))
                    elif (col == "#A3FFFA"):
                        category = Literal("social networking", datatype=XSD.string)
                        graph_dump.add((observedID, obs.category, category))
                    elif (col == "#F6DEE7"):
                        category = Literal("life sciences", datatype=XSD.string)
                        graph_dump.add((observedID, obs.category, category))
                    elif (col == "#CEFFEB"):
                        category = Literal("government", datatype=XSD.string)
                        graph_dump.add((observedID, obs.category, category))
                    elif (col == "#E8FED3"):
                        category = Literal("publications", datatype=XSD.string)
                        graph_dump.add((observedID, obs.category, category))
                    elif (col == "#FEF1B6"):
                        category = Literal("geographic", datatype=XSD.string)
                        graph_dump.add((observedID, obs.category, category))    
                    elif (col == "#E8EDFF"):
                        category = Literal("media", datatype=XSD.string)
                        graph_dump.add((observedID, obs.category, category))
                    elif (col == "#EAF7F6"):
                        category = Literal("cross domain", datatype=XSD.string)
                        graph_dump.add((observedID, obs.category, category))
                    elif (col == "#FEEDDF"):
                        category = Literal("user-generated content", datatype=XSD.string)
                        graph_dump.add((observedID, obs.category, category))
        ...
        graph_dump.serialize("observed_withCategories.ttl", format="turtle")